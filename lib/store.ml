...
module type StoreType = sig
  ...
  val map :
    (FrequencyBagGoods.t -> FrequencyBagGoods.t) ->
    FrequencyBagGoods.t list ->
    FrequencyBagGoods.t list

  val filter :
    (FrequencyBagGoods.t -> bool) ->
    FrequencyBagGoods.t list ->
    FrequencyBagGoods.t list
...
module Store : StoreType = struct
  ...
  (**Given a list of FrequencyBagGoods [b_lst] and a store [st], add all goods
     of [bg] to the store [st].*)
  ...
  let rec get_del_items bg =
    ...
    | h :: t ->
        ...
        else get_del_items (FrequencyBagGoods.of_list t)

  (*[del_items bg] deletes items of a bag [bg] if their quantity is less than or
    equal to zero.*)
  let rec del_items bg =
    ...
    | h :: t ->
        if Item.get_quantity h <= 0 then del_items (FrequencyBagGoods.of_list t)
        else
          FrequencyBagGoods.join
            (FrequencyBagGoods.of_list [ h ])
            (del_items (FrequencyBagGoods.of_list t))

  (*[store_change_quan dec item bg] sells [dec] amount of [item] in [bg] and
    returns a new bag with the updated amount of [item] *)
  let rec store_change_quan dec (item : Item.t) bg =
    match FrequencyBagGoods.to_list bg with
    | [] -> FrequencyBagGoods.empty
    | h :: t ->
        if Item.get_name item == Item.get_name h then
          let new_item = Item.change_quantity h (-1 * dec) in
          FrequencyBagGoods.of_list (new_item :: t)
        else
          FrequencyBagGoods.join
            (FrequencyBagGoods.of_list [ h ])
            (store_change_quan dec item (FrequencyBagGoods.of_list t))

  (**Sell [dec] amount of an item [item] within a given store [st]. Requires
     that [b] is a good in [st] already.*)
  let rec sell_goods dec (item : Item.t) st =
    match st with
    | [] -> []
    | h :: t ->
        let b1 = store_change_quan dec item h in
        let b2 = del_items b1 in
        b2 :: sell_goods dec item t
  ...
  let rec filter (f : FrequencyBagGoods.t -> bool) st = List.filter f st
  ...
  let rec item_by_name nam st =
    ...
    let rec r11 nam lst =
      ...
      | [] -> None
      | h :: t ->
          let r1 = item_by_nam_aux nam h in
