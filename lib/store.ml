...
module type StoreType = sig
  ...
  val count_products : t -> int
  val all_products : t -> t
  val import_goods : t -> t
  val sell_goods : t -> t
  val popular_goods : t -> t
  val on_sale_goods : t -> t
  val in_stock_goods : t -> t
  val out_of_stock_goods : t
end

module Store (Bag : SampleGoodsType) : StoreType = struct
  module Bag : SampleGoodsType = StoreProducts
    ...
    | h :: t ->
        let h =
          let update_bag b =
            ...
            | [] -> []
            | h1 :: t1 ->
                if h1.get_name () == b then
                  if h1.get_quantity () - dec > 0 then
                    h1.change_quantity (h1.get_quantity () - dec) :: t1
                  else t1
                else h1 :: update_bag b t1
          in
          update_bag b
        in
        h :: sell_goods dec b t

  let determine_popular bag_goods lim =
    match bag_goods with
    | [] -> []
    | h :: t ->
        if h.get_quantity () <= lim then h :: determine_popular t lim
        else determine_popular t lim

  let popular_goods lim st =
    match st with
    | [] -> []
