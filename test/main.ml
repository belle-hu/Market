...
let updated_fruits_bag =
  BagOfGoods.of_list [ apple_item_a; grapes_item; orange_item ]
...
let store_sell_goods_test msg out in1 = msg >:: fun _ -> assert_equal out in1
...
let store_tests =
  [
    ...
    store_sell_goods_test
      "store sell_goods_test for item apple in store of one bag" updated_store1
      (Store.sell_goods 1 apple_item store1);
    store_sell_goods_test "store sell_goods_test for item baseball "
      store2_updated
      (Store.sell_goods 10 baseball store2);
  ]
