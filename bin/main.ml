...
let  map_to_string mp = 
  ...
  match bd with 
  | [] -> ""
  | (p1, p2):: t -> "Category: " ^ p1 ^ FrequencyBagGoods.to_string p2 ^ "|"
  ^ (r1 t)
  in r1 bd
...
(*[change_price_aux nam price_change cat cat_mp] is a helper method that changes the
   price of an item of name [nam] in a category [cat] in our category map [cat_mp]*)
let change_price_aux nam price_change cat cat_mp = 
  match (StringMap.find_opt cat cat_mp) with 
  | None -> failwith "Category not found!"
  | Some v -> let v' = FrequencyBagGoods.update_price v nam price_change in 
...
let change_price () =
        print_endline
          "\n\
           Please enter the name, the price_change, and category of the item you want to change \
           in the format of \"[name] [price_change] [category]\" (eg. \"apple 2 fruit\"). \n\
...
(*[change_quantity_aux nam quantity_change cat cat_mp] is a helper method that changes the
   quantity of an item of name [nam] in a category [cat] in our category map [cat_mp]*)
   let change_quantity_aux nam quantity_change cat cat_mp = 
    match (StringMap.find_opt cat cat_mp) with 
    | None -> failwith "Category not found!"
    | Some v -> let v' = FrequencyBagGoods.update_quantity v nam quantity_change in 
    let m' = StringMap.add cat v' cat_mp in 
    category_map := m';
    our_store := Store.of_list (category_list !category_map)
let change_quantity () =
  print_endline
    "\n\
     Please enter the name, the quantity, and the category of the item you want to import in \
     the format of \"[name] [imported_quantity] [category]\" (eg. \"apple 2 fruit\"). \n\
...
(*[add_to_category cat bg] adds binding key [cat] to value [bg] 
in map [cat_mp]*)
let add_to_category cat bg = 
  let v1 = StringMap.find_opt cat !category_map in 
  match v1 with 
  | None -> category_map := StringMap.add cat bg !category_map;
  our_store := Store.of_list (category_list !category_map)
  | Some v -> let joined_bg = FrequencyBagGoods.join v bg in
category_map := StringMap.add cat joined_bg !category_map;
our_store := Store.of_list (category_list !category_map)

(**Choice 1*)
let create_item_add_to_store () =
  print_endline
    "\n\
     Please enter the name, price, quantity, category of the item you want to create \
     in the format of \"[name] [price] [quantity] [category]\" (eg. \"apple 1 2 fruit\")\n\
     ...
     previously created \"apple 1 2 fruit\", please do not create a new \"apple 5 \
     7 fruit\". Instead, you should use the change_price or change_quantity \
     functionalities. \n\
  ...
  | [ nam; pri; quan; cat ] ->
      ...
      print_endline
        ...
        ^ " and quantity: "
        ^ string_of_int (Item.get_quantity item1) ^ " in " ^ cat);
...
(*Choice 5*)
let show_category_and_items () =
  print_endline (map_to_string !category_map)
...
(*Choice 9*)
(*[sell_items_aux cat nam quan_change cat_map] is a helper function that sells [quan_change] 
   of an item by name [nam] in store*)
let sell_items_aux cat nam quan_change st = 
 ...
 | Some i -> our_store := Store.sell_goods quan_change i st;
 match (StringMap.find_opt cat !category_map) with 
    | None -> failwith "Category not found!"
    | Some v -> let v' = FrequencyBagGoods.update_quantity v nam (1*quan_change) in 
    let m' = StringMap.add cat v' !category_map in 
    category_map := m'

let sell_items () =
  print_endline "\n\
     Please enter the name, the quantity, and the category of the item you want to remove \
     in the format of \"[name] [quantity_removed] [category] \" (eg. \"apple 1 fruit\"). \n\
    \ The new quantity of the item will be original quantity - \
     quantity_removed. \n\
  ...
  | [ nam; quantity_change; cat ] ->
      ...
      print_endline
        ("Successfully sold item: " ^ nam ^ " of "
       ^ quantity_change ^ " amount" ^ " in " ^ cat);
  | _ -> failwith "Invalid input"
...
let customer_buy () =
  print_endline
    "Please enter the name, the quantity, and the category of the item you want to buy in the \
     format of \"[name] [quantity] [category]\" (eg. \"apple 1 fruit\"):";
  print_endline
    "Notice: 
...
let rec work () =
  ...
  print_endline
    "Your store can do several things!\n\
     ...
     5. Show all items with their categories in store \n\
     6. Show all item names in the store \n\
     7. Show total quantity of all items in the store \n\
     8. Show total cost of all items in the store  \n\
     9. Remove items \n\
     10. Take a look at our most popular items! Almost sold out! \n\
     11. Switch to customer mode \n\
     12. Show transaction history \n\
     Q. Quit";
  print_endline "\nPlease enter your choice of 1-12, or Q\n";
  match read_line () with
  | "1" ->
